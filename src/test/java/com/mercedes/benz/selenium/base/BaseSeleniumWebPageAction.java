...
public abstract class BaseSeleniumWebPageAction extends Common{
     ...
     * @param driver
     */
    public BaseSeleniumWebPageAction(WebDriver driver, final WebDriverWait wait) {
    	this.driver=driver;
        this._wait = wait;
    ...
    public void click(final WebDriver driver, final By buttonLocator) throws InterruptedException {
       ...
       // _wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(buttonLocator));
        if (_wait.until(ExpectedConditions.elementToBeClickable(buttonLocator)).isDisplayed()) {
            _wait.until(ExpectedConditions.elementToBeClickable(buttonLocator)).click();
        } else {

            final WebElement element = _wait.until(ExpectedConditions.elementToBeClickable(buttonLocator));
           ...
           // driverSleep(500, ChronoUnit.MILLIS);
            ...
            if (ExpectedConditions.stalenessOf(element).apply(driver) == null
     ...
     * waits for element to load for 10 seconds
     * if element found before that then wait is exited
     *
     * @param WebEElement locator
     */
    public static void explicitWait(final By locator) {
